...
/**
 * Desc: 使用个体模式的循环（looping）模式
 */
public class SuspiciouslyLoginDetectTest2 {
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<LoginEventBean> sourceDS = streamEnv.fromElements(
                ...
                new LoginEventBean("user-1", "ip-1", "success", 2000L),
        ...
        patternStream.select(new PatternSelectFunction<LoginEventBean, String>() {
            ...
            public String select(Map<String, List<LoginEventBean>> map) throws Exception {
                ...
                List<LoginEventBean> firstList = map.get("first");
                return firstList.get(0).getUserId() + "连续三次登录失败，" + "登录事件分别为："
                        + firstList.get(0).getTimestamp() + ","
                        + firstList.get(1).getTimestamp() + ","
                        + firstList.get(2).getTimestamp();
            }
        }).print();
